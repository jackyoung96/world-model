...
def train(args):
    ...
    if args.tb_log:
        if not os.path.isdir('tb_log'):
            os.mkdir('tb_log')
        dtime = datetime.now()
        writer = SummaryWriter(os.path.join('tb_log', env_name, 'ppo', dtime.strftime("%y%b%d%H%M%S")))
    if randomize:
        dyn_range = {
            # cartpole
    ...
    for i_episode in range(args.epoch+1):
        ...
        if i_episode % 100 == 0:
            ...
            print(total_rewards)
            torch.save(agent.policy.state_dict(), "save/ppo/policy_%s.pth"%env_name)

        if np.mean(scores_window)>=best_score:    
            torch.save(agent.policy.state_dict(), "save/ppo/policy_%s_best.pth"%env_name)
